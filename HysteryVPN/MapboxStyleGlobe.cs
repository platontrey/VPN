...
namespace HysteryVPN
{
    public class MapboxStyleGlobe : HelixViewport3D
    {
        ...
        private void CreateSkybox()
        {
            ...
            var starTexture = generator.GenerateStarField(1024, 512, 10000); // 2:1 aspect для сферы
        ...
        {
            var texture = GenerateGeoJsonTexture(4096, 2048);
        ...
        private WriteableBitmap GenerateGeoJsonTexture(int width, int height)
        {
            ...
            {
                // Заливка фона (вода)
                drawingContext.DrawRectangle(new SolidColorBrush(Color.FromRgb(10, 14, 20)), null, new Rect(0, 0, width, height));

                // Загрузка GeoJSON
                try
                {
                    string json = File.ReadAllText("countries.geojson");
                            ...
                            if (feature.Geometry.Type == "MultiPolygon" || feature.Geometry.Type == "Polygon")
                            {
                                ...
                                foreach (var polygon in polygons)
                                {
                                    if (polygon.Count > 1)
                                    {
                                        ...
                                        // Преобразовать 3D точки в 2D UV для текстуры
                                        var points2D = new List<Point>();
                                        foreach (var point3D in polygon)
                                        {
                                            var uv = Point3DToUV(point3D, width, height);
                                            points2D.Add(uv);
                                        }

                                        if (points2D.Count > 0)
                                        {
                                            pathFigure.StartPoint = points2D[0];
                                            for (int i = 1; i < points2D.Count; i++)
                                            {
                                                pathFigure.Segments.Add(new System.Windows.Media.LineSegment(points2D[i], true));
                                            }
                                        }
                                        ...
                                        // Рисуем заливку сушы
                                        drawingContext.DrawGeometry(new SolidColorBrush(Color.FromRgb(25, 30, 38)), null, pathGeometry);
        ...
        private Point Point3DToUV(Point3D point, int width, int height)
        {
            // Преобразование 3D точки на сфере в UV координаты текстуры
            double u = (Math.Atan2(point.Z, point.X) + Math.PI) / (2 * Math.PI);
            double v = Math.Acos(point.Y) / Math.PI;

            return new Point(u * width, v * height);
        }

        private List<List<Point3D>> ParseGeometry(GeoJsonGeometry geometry)
        {
            ...
            try
            {
                if (geometry.Type == "MultiPolygon")
                {
                    ...
                    foreach (var polygonArray in geometry.Coordinates.EnumerateArray())
                    {
                        foreach (var ringArray in polygonArray.EnumerateArray())
                        {
                            var points = new List<Point3D>();
                            foreach (var coordArray in ringArray.EnumerateArray())
                            {
                                var coords = coordArray.EnumerateArray().ToArray();
                                if (coords.Length >= 2)
                                {
                                    double lon = coords[0].GetDouble();
                                    double lat = coords[1].GetDouble();
                                    var point3D = LatLonToPoint3D(lat, lon, 1.0);
                                    points.Add(point3D);
                                }
                            }
                            if (points.Count > 1)
                ...
                {
                    // Polygon: [[[lon, lat], ...]]
                    foreach (var ringArray in geometry.Coordinates.EnumerateArray())
                    {
                        var points = new List<Point3D>();
                        foreach (var coordArray in ringArray.EnumerateArray())
                        {
        ...
        private void SetupLighting()
        {
            ...
            ambientLight = new AmbientLight(Color.FromRgb(5, 5, 10));
            ...
            backLight = new DirectionalLight(Color.FromRgb(50, 50, 70), new Vector3D(0.5, 0.2, 1));
        ...
        {
            var point3D = LatLonToPoint3D(lat, lon, 1.0); // На поверхности

            var userPoint = new SphereVisual3D
            {
                Radius = 0.02,
                Center = point3D,
                Material = new EmissiveMaterial(new SolidColorBrush(Colors.Red)),
                BackMaterial = new EmissiveMaterial(new SolidColorBrush(Colors.Red))
            };

            Children.Add(userPoint);
        }
